...
def create_expense(group_id: int, payload):
    ...
    original_currency_code = payload.original_currency_code or payload.currency_code
    original_amount_minor = payload.original_amount_minor or payload.amount_paise
    exchange_rate_ppm = payload.exchange_rate_ppm or 1000000
    if payload.currency_code != group["currency_code"]:
        raise ApiError("Converted expenses must be saved in the group's base currency.")
    ...
    try:
        ...
        _record_activity(
            ...
            payload.created_by_user_id,
            "expense_created",
            f"Added {payload.description} for {format_paise(payload.amount_paise, payload.currency_code)}",
            {"expense_id": expense_id, "source_type": payload.source_type},
        )
        db.commit()
    except Exception:
        db.rollback()
...
def _hydrate_expenses(expense_rows):
    ...
    for row in expense_rows:
        expense = dict(row)
        expense["amount_display"] = format_paise(expense["amount_paise"], expense["currency_code"])
        if expense.get("original_currency_code") and expense["original_currency_code"] != expense["currency_code"]:
            expense["original_amount_display"] = format_paise(
...
def _record_activity(group_id: int, actor_user_id: int | None, event_type: str, summary: str, metadata=None):
    get_db().execute(
        """
        INSERT INTO activity_events (group_id, actor_user_id, event_type, summary, metadata_json)
        VALUES (?, ?, ?, ?, ?)
        """,
        (group_id, actor_user_id, event_type, summary, json.dumps(metadata or {})),
    )
